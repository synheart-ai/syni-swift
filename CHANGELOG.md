...
### Changed (BREAKING — rename, no code change)

- **Renamed product `SyniSwift` → `Syni`**. The `Swift` suffix was
  redundant noise (it's a Swift package — of course it's Swift).
  Matches the naming of pub.dev `syni` and Maven Central
  `ai.synheart:syni`. Affects:
  - SwiftPM: `.product(name: "Syni", package: "syni-swift")` and
    `import Syni`
  - CocoaPods: `pod 'Syni', '~> 0.0.4'`
  - Internal source folder: `Sources/SyniSwift/` → `Sources/Syni/`
- The orphaned `SyniSwift 0.0.3` pod on CocoaPods and the `SyniSwift`
  SwiftPM product at the v0.0.3 tag stay as-is but receive no further
  updates.
...
- **CocoaPods support** — `pod 'Syni', '~> 0.0.3'` now works
  alongside SwiftPM. Same source tree, same dlsym runtime resolution.
  The podspec's `prepare_command` symlinks
  `<app>/synheart/vendor/syni-runtime/SyniRuntime.xcframework` into the
  pod install directory, mirroring the pattern used by syni-flutter.
...
- Native runtime distribution: dropped `Package.swift` `binaryTarget`.
  Consumers install `syni-runtime` once with
