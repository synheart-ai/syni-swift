...
Then `pod install`. Like the SwiftPM install, the pod ships Swift
sources only — see [Native runtime install](#native-runtime-install)
below for wiring the `SyniRuntime.xcframework` into your Xcode target.
...
`Syni` is a thin Swift wrapper around the `syni-runtime` native
core. The runtime is shipped via the `synheart` CLI — install it once
into your project:
...
| Android | `ai.synheart:syni` (Maven Central) | [syni-kotlin](https://github.com/synheart-ai/syni-kotlin) |
